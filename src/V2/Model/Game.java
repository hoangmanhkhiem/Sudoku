...
//            6 9 2 3 5 1 8 7 4
//            7 4 5 2 8 6 3 1 9
    private boolean[][] isConst = new boolean[length][length];

    public Game() {
        board_game = new Node[length][length];
        Random random = new Random();
        for (int i = 0; i < length; i++) {
            for (int j = 0; j < length; j++) {
                if(random.nextBoolean()){
                    board_game[i][j] = new Node(i,j,answer[i][j]);
                    isConst[i][j] = true;
                } else {
                    board_game[i][j] = new Node(i,j,0);
                    isConst[i][j] = false;
                }
            }
        }
